...
hurtFox:function(){ //Called when Fox's body intersects another entity
    this.FoxHealth = this.FoxHealth - 10;
    f('health').style.width = this.FoxHealth;
...
scoreMarble:function(t){
    ...
    this.ScorePts(10);
    this.FoxHealth++; //heal fox 1 pt every marble
},
starit:function(a){
	this.ScorePts(40);	
	fvx = (this.Score/10%5);
...
CreateWorldDefinitions:function(Sprites){
	...
	for(r=0;r<Sprites.length;r++){    //this part takes our level's input and generates the x,y, and z coords of objects in thw world
		...
		if(sprite.type=="bird"){
			...
			sprite.src="images/storm-cloud.gif";
			sprite.width=200;
			sprite.height=300;
			sprite.y = -10;
		}
    ...
    if((((this.LengthOfCourse - sprite.zq)+70)>=this.LengthOfCourse-this.FoxZ)&&sprite.type=="bluewave"){
        ...
        if((((this.LengthOfCourse - sprite.zq)+60)==this.LengthOfCourse-this.FoxZ)&&sprite.death==0){
            ...
            if(HurtX==1){ 
                ...
                else if(sprite.type =="castle" || sprite.type == "welcome" || sprite.type =="left" || sprite.type =="right" ||sprite.type=="bluewave" ||	sprite.type =="forward" ||	sprite.type =="zap"  || sprite.type =="getem"){}
                else
                	this.hurtFox();
                if(sprite.type == "redmarble")
                	this.scoreMarble(t);
...
SetEffectVolume:function(x){
	this.EffectsVolume = parseFloat(x);
...
HurtFoxSound:function(){
	var file = "audio/bop"+this.ext;
	painSound = new Audio(file);
	painSound.volume = this.EffectsVolume;
    painSound.play();
}
