...
f(hold.id).style.bottom = window.goldenHorizon - Height;//once MidHeight
}
f("d"+hold.id).style.zIndex = (LengthOfCourse - hold.zq)+60;
Leftx =  (TwoFourtyConstant +(NormX*dW*-1)) + hold.x*dW - (Width - hold.ow)/2;	
...
if(hold.type=="bird"){f("d"+ hold.id).style.top = Topy;
	...
	zscale = Math.round((hold.z/10) -10);
	zs = zscale + "%"; //zs looks like tidal wave
	dws = 49 - Math.round(dW*49);
	dwsp = dws + "%";
	d48 = Math.round(48 + (hold.y*dW));
	d48p = d48 + "%"; //straight 48 works pretty good, I want to mod it though from the hold.y var
f("d"+hold.id).style.top = d48p;//window.goldenHorizon - MidHeight + Math.round(hold.y*dW);//dwsp;//zs;//"50%";//window.goldenHorizon - MidHeight + hold.y;          
}
